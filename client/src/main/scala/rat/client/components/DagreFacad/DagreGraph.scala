...
//package grapheditor.client.components.graph.DagreFacad {
//import rat.shared.SharedUtil

import scala.scalajs.js
import js.annotation._
package rat.client.components {

  //import rat.client.components.GraphUtil
  //@ScalaJSDefined
  ...
  object GraphNode {
    ...
    def apply(label: String, width: Double, height: Double, x: Double, y: Double): GraphNode = {
      js.Dynamic.literal(label = label, width = width, height = height, x = x, y = y).asInstanceOf[GraphNode]
    }

    def apply(label: String, width: Double, height: Double): GraphNode = {
      js.Dynamic.literal(label = label, width = width, height = height).asInstanceOf[GraphNode]
    }

    def apply(label: String): GraphNode = {
      val a = GraphUtil.textBoundingBox(Array(label))
  ...
  @js.native
  object Dagre extends js.GlobalScope {
    val dagre: DagreFactory = js.native
  }
